...
| [L-04](#l-04-signature-can-be-re-used-if-the-game-is-deployed-on-another-chain) | Signature can be re-used if the game is deployed on another chain | Low |
...
[FighterFarm::_createNewFighter](https://github.com/code-423n4/2024-02-ai-arena/blob/main/src/FighterFarm.sol#L509)

```solidity
509:        bool dendroidBool = fighterType == 1;
```

User's are able to redeem their `mintpasses` via the `redeemMintPass` function. This function only validates the `mintPassId` (tokenId of mintpass). No other values, including the `fighterType` are validated:

[FighterFarm::redeemMintPass](https://github.com/code-423n4/2024-02-ai-arena/blob/main/src/FighterFarm.sol#L233-L250)
...
As we can see above, a user is able to pass arbitrary values for any of the calldata parameters (with the exception of the `mintpassIdsToBurn`) without those values being validated. Specifically, this allows the user to pass in a `fighterType == 1`, which will guarantee that the user will mint a `dendroid`, even if the metadata of their `mintpass` specifies a `champion`.

### Impact
Any user with a `mintPass` can ensure that the fighter(s) they mint via `redeemMintPass` will be a `dendroid`, and therefore have more value than the metadata of their `mintPass` specifies.

### Recommended Mitigation Steps
All calldata inputs should be validated to ensure that a user is only able to mint the correct fighter type.

## [L-04] Signature can be re-used if the game is deployed on another chain

### Bug Description / Proof of Concept
When a user mints a fighter via `FighterFarm::claimFighters`, the user must supply a signature from the server. The signature digest that the server signed is made up of the user's address (msg.sender), calldata inputs, and state that pertains to the user:

[FighterFarm::claimFighters](https://github.com/code-423n4/2024-02-ai-arena/blob/main/src/FighterFarm.sol#L199-L206)
...
202:            numToMint[1],
203:            nftsClaimed[msg.sender][0], // @audit: state that changes after mint
204:            nftsClaimed[msg.sender][1]
205:        )));
206:        require(Verification.verify(msgHash, signature, _delegatedAddress));
```

As we can see above, a `deadline` or `chainId` is not part of the signature digest. Therefore, a user is able to acquire this signature via the front-end, use it to mint via the above function, and then re-use the signature at a future time if the game is deployed on a separate chain.

### Impact
A user can potentially re-use server signatures in the future in order to mint free fighters on different chains that the game gets deployed on. Note that the assumptions here are that the `_delegatedAddress` will be the same across all the chains and that the game is deployed on another chain in the first place.
